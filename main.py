...
def show_all_docs_info():
    doc = []
    for current_document in documents:
        doc.append(show_document_info(current_document))
    return doc
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            new_doc_shelf_numb = add_new_doc(new_doc_number, new_doc_type, new_doc_owner_name, new_doc_shelf_number)
            print('\nНа полку "{}" добавлен новый документ'.format(new_doc_shelf_numb))
        elif user_command == 'd':
            user_doc_number = input('Введите номер документа - ')
